...
const EditPageForm = () => {
  ...
  const updatePageMutation = useMutation({
    mutationFn: async (data: PageFormData) => {
      const response = await axios.put(`http://localhost:4000/api/pages/${id}`, data);
      return response.data;
    },
    onSuccess: () => {
  ...
  if (error) {
    return <div className="text-center py-8 text-red-600">Failed to load page</div>;
  }

  return (
    <div className="max-w-2xl mx-auto">
      <h2 className="text-2xl font-semibold text-gray-900 mb-6">Edit Page</h2>
